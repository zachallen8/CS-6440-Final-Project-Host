...
import { mapToFhirPatient } from '../utils/fhir';
...
function Home() {
  ...
  useEffect(() => {
    ...
    async function fetchPatient() {
      try {
        ...
        if (mounted) {
          const fhir = mapToFhirPatient(data)
          setFhirPatient(fhir)

          const conception = data?.conception_date ?? fhir?.extension?.find(e => e.url && e.url.endsWith('conceptionDate'))?.valueDate
          if (conception) {
            try {
              const weeks = computeWeeksFromConception(conception)
  ...
  const displayName = loadingPatient ? 'You' : (fhirPatient?.name?.[0]?.given?.join(' ') || fhirPatient?.name?.[0]?.family || 'You')
  ...
  <PregnancyProgress weeks={patientWeeks !== null ? patientWeeks : 22} name={displayName} />
