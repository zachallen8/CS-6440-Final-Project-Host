...
function mapFhirCommunication(msg, convMeta) {
  const isPatientSender = msg.sender_type === 'patient';
  const patientRef = `Patient/${convMeta.patientId}`;
  const providerRef = `Practitioner/${convMeta.providerId}`;
  const senderRef = isPatientSender ? patientRef : providerRef;
  const recipientRef = isPatientSender ? providerRef : patientRef;
...
function buildConvBundle(convMeta, fhirComms) {
  return {
    resourceType: 'Bundle',
    type: 'collection',
    entry: (fhirComms || []).map(resource => ({ resource}))
  };
}
...
function Messaging() {
  ...
  const [fhirMessages, setFhirMessages] = useState({});
  ...
  const fetchMessages = async (conversationId) => {
    try {
      ...
      const fhirComms = convMeta ? data.map(msg => mapFhirCommunication(msg, convMeta)) : [];
      ...
      setFhirMessages(prev => ({
        ...prev, [conversationId]: fhirComms,
      }));
    } catch (error) {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const convMeta = conversations.find(c => c.id === selectConversation);
      if (convMeta && newMessage) {
        const fhirComm = mapFhirCommunication(newMessage, convMeta);
        setFhirMessages(prev => ({
          ...prev,
          [selectConversation]: [...(prev[selectConversation] || []), fhirComm]
        }));
      }
